...
from opensilex_python_client.variables._component_resolver import find_or_create_component
...
def test_find_or_create_unit_exists(mock_client):
    with patch("opensilex_python_client.variables._component_resolver.VariablesApi") as MockVariablesApi:
        ...
        result = find_or_create_component(ctx, "unit", None, name)
        assert result == uri
...
def test_find_or_create_entity_exists(mock_client):
    with patch("opensilex_python_client.variables._component_resolver.VariablesApi") as MockVariablesApi:
        ...
        ctx = VariablesContext(client=mock_client)
        result = find_or_create_component(ctx, "entity", None, name)
        assert result == uri
...
def test_find_or_create_characteristic_exists(mock_client):
    with patch("opensilex_python_client.variables._component_resolver.VariablesApi") as MockVariablesApi:
        ...
        ctx = VariablesContext(client=mock_client)
        result = find_or_create_component(ctx, "characteristic", None, name)
        assert result == uri
...
def test_find_or_create_method_exists(mock_client):
    with patch("opensilex_python_client.variables._component_resolver.VariablesApi") as MockVariablesApi:
        ...
        ctx = VariablesContext(client=mock_client)
        result = find_or_create_component(ctx, "method", None, name)
        assert result == uri
